...
Status: **M59 — the station shows the path to real money.** A `/mandate` page shows the company's own conditions for asking to trade real money, as last assessed, and every scheme's paper record after costs. Before it, M58: a method is judged by the mechanisms too.** Evolution now also reads each agent's discovery record: a method under which five or more mechanisms were decided and none became a scheme is rewritten, by a colleague whose mechanisms thrive. Before it, M57: capital follows the record.** Each scheme's share of the paper book comes from its record after costs, re-read daily by the Portfolio Manager: 5% while gathering, 10% when earning, nothing when losing, schemes together at most half the book. Before it, M56: every department works, every day.** Judging and discovery no longer stop at their first success, and each idle department holds a daily duty run by its own agent: the audit, the integrity check, the health check, a lesson and the research director's memo every seat reads. Before it, M55: money is judged after costs, by episode.** A candidate scheme's paper round trips after fees are grouped by independent episode and counted; a scheme earns only beyond the bar for the family of schemes, one losing after costs stops opening positions, and the mandate gains a fourteenth condition, `earning`. Before it, M54: the company chooses whom it follows.** Every X account, Telegram channel and Discord channel the company has read is measured by whether its posts come before its instrument beats its peers or after, counted by episode and at a bar divided by how many voices were measured; once a day a market-intelligence agent follows and drops from that record. Before it, M53: a changed catalogue is a new question.** Standing duties run whenever they have work, so the agents are asked about new sources at the next wake. Before it, M52: the station shows the company at work.** The facility is lit by who acted in the last ten minutes, read off the ledger, with the current wake, today's counts, who is doing what, and a live feed of every decision, refreshed in place. Before it, M51: X and Discord, signed in once.** A dedicated Edge profile, signed into by the person with `aurelis social login`, lets read-only readers take each followed memecoin's own X account, rotating cashtag searches, and followed Discord channels. Before it, M50: the company follows social media through the device.** Reddit is read from its own feed with no app, Telegram public channels through their web preview with no account, and each followed memecoin's own X and Telegram channels are followed automatically. Cashtags such as `$MOON` now count as mentions. The readers are read-only code; no model drives a browser. Before it, M49: the judge sees what the mechanisms see.** The agents asked for it: the first question, which market to judge, now shows each market's book, flow, funding and open-interest readings, what fired on it in the last day, and the open calls the company's mechanisms have on it. Before it, M48: the company evolves how its agents think.** Every judging agent carries a written, versioned method that is part of its identity at every seat. Once a day the company measures each method by the forward Brier of the views sealed under it. When a method is worse than a coin toss beyond its error, the best-calibrated colleague writes a replacement, or the agent revises its own if nobody is beating chance. Before it, M47: the company works while there is work.** Declines are remembered per market, a mechanism's predictions no longer block its author, the miner is fifty times faster and asks about rare attention events first, a declined pattern returns only on new evidence, and the day's budget is spread across wakes. Before it, M46: the shared brain.** Every agent reads one brain before it answers: the record of mechanisms, calibration, retirements and declines, and the notes agents and the operator leave. It is rendered as an Obsidian vault every wake, and the operator writes into it by dropping a file in its Inbox. Before it, M45: the service survives what killed it.** Prices are stored exactly, a failed paper order is rolled back alone, a failed wake is an incident rather than the end of the service, token positions are capped by their pool, and a supervisor script restarts the service and starts it at logon. Before it, M44: the memecoin desk opens.** A token's bars come from its deepest pool, keyed by chain and contract so its attention and its price sit on one entity; a dex grant names networks and a following rule the wake evaluates; a paper fill pays its desk's own costs; a mechanism is stated on the desk its pattern fired on. Before it, M43: sources for every market, chosen by the agents, keyed by a person.** The catalogue is twenty free, official sources across seven kinds and every desk — publishers' feeds, Stocktwits and Bluesky per instrument, DEX Screener boosts and GeckoTerminal trending on token entities, Reddit and CryptoPanic with a key the operator sets in the environment; the agents choose per market, and the wake says which key is still missing. Before it, M42: a reading is not a trigger, and the book closes flat.** The kinds the service records every wake as readings are no longer offered as triggers, a mechanism stated on one is retired with the reason, the in-sample baseline spans only the trigger's own recording, a paper close is sized by the units the open bought, and the wake flattens whatever a close left behind. Before it, M41: the agents ask for the sources they need.** The catalogue is four publishers' official, free, keyless feeds; a market-intelligence agent asks for the ones it wants and says why, the service reads them under a grant a person records once, and every headline becomes an event on the instruments it names, with bursts derived. Before it, M40: paper fills at the price the wake can see.** A paper order placed an hour after a trigger no longer fills at the trigger's close: it fills at the newest close the wake could see, the slippage is on the record, and a firing the wake cannot yet see a price for is refused. Before it, M39: evidence is counted by independent episode.** The first live candidate scheme cleared the bar on twenty-six predictions from one afternoon across thirty correlated instruments; predictions whose horizons overlap are now one episode, a mechanism needs ten of them as well as twenty predictions and must beat the drift by both, and a paper position opened while it qualified closes at its horizon whatever the record says since. Before it, M38: a mechanism may fire on the conjunction it was shown.** The discovery form carries `FIRES_ON: trigger | conjunction`; on `conjunction` a prediction seals only when the second event follows the trigger inside the window, at that instant, so the mechanism is tested on the pattern the agent actually reasoned about. Before it, M37: the facility in pixels.** The station is the brief's
...
| **M41** ✅ | **The agents choose sources** | a catalogue of free, official, keyless feeds; a market-intelligence agent asks for the ones it wants, with a reason; headlines become events on the instruments they name |
| **M42** ✅ | **Readings and flat books** | a reading the service takes every wake is not a trigger; a mechanism stated on one is retired with the reason; the in-sample baseline spans the trigger's own recording; closes are sized by units held and residuals are flattened |
| **M43** ✅ | **Sources for every market** | twenty free, official sources across seven kinds and every desk; social posts, paid boosts and trending pools as events; keys a person supplies in the environment, never on the record; the agents choose per market |
| **M44** ✅ | **The memecoin desk opens** | a price recording per token from its deepest pool; a dex grant names networks and a following rule; fills pay the desk's own costs; a mechanism is stated on the desk its pattern fired on |
| **M45** ✅ | **The service survives** | exact prices; a failed order or mechanism is rolled back alone; a failed wake is an incident and the next runs; token positions capped at 2% of the pool; a supervisor that restarts the service and starts at logon |
| **M46** ✅ | **The shared brain** | every seat reads one brain: the record's mechanisms, calibration, retirements and declines, and the notes agents leave with a NOTE line; rendered as an Obsidian vault every wake; the operator writes in through its Inbox |
| **M47** ✅ | **Work while there is work** | per-market decline memory; mechanism predictions are not views; a one-pass miner that asks about rare events first; re-asked only on new evidence; a per-wake share of the day's budget |
| **M48** ✅ | **The company evolves how its agents think** | a written, versioned method per agent at every seat; fitness is the forward Brier since adoption against a coin toss; once a day a failing method is rewritten by the best-calibrated colleague, or revised by the agent itself |
| **M49** ✅ | **The judge sees what the mechanisms see** | each market's line at the first question carries its fresh book/flow/funding/open-interest readings, the signals that fired in the last day, and the open mechanism calls on it, as of what the company knew |
...
| **M54** ✅ | **The company chooses whom it follows** | each voice's posts against the 24h move after and before, peer-adjusted, by episode, with a sign test at a bar divided by the voices measured; a daily curation seat follows and drops from a closed set; `aurelis social voices` and `/voices` |
| **M55** ✅ | **Money is judged after costs** | paper round trips after fees, by independent episode, by sign test, with drawdown; earning at the family bar, losing at an undivided one; a losing scheme is suspended by Risk; the mandate reads `earning` |
| **M56** ✅ | **Every department works, every day** | judging and discovery are standing duties; audit, integrity and health checks daily in software; a daily lesson and memo into the shared brain, withheld if they cite an unshown figure; `aurelis duty` |
| **M57** ✅ | **Capital follows the record** | a declared ladder from the after-costs record sizes each scheme; positions sized from the live allocation; the Portfolio Manager withdraws and re-allocates daily; schemes capped at half the book |
| **M58** ✅ | **A method is judged by the mechanisms too** | discovery fitness from the mechanisms decided under each method; a failing discovery record rewrites the method, taught by a colleague whose mechanisms thrive |
| **M59** ✅ | **The station shows the path to real money** | `/mandate`: the latest assessment condition by condition, every scheme's record after costs, the verdict history |
