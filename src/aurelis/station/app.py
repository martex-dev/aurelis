...
class StationApp:
    ...
    def _service(self, _rest: list[str], _q: dict[str, list[str]]) -> Response:
        with self.runtime.database.session() as session:
            return self._render("Service", pages.service_page(session), "")
    ...
    def _workshop(self, _rest: list[str], _q: dict[str, list[str]]) -> Response:
        with self.runtime.database.session() as session:
            return self._render("Workshop", pages.workshop_page(session), "")
    ...
    def _brain(self, _rest: list[str], _q: dict[str, list[str]]) -> Response:
        with self.runtime.database.session() as session:
